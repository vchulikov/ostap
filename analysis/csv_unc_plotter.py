...
def complex_histo_draw(file_name, epidlc, epidsc, epidlc_bin, epidsc_bin, epidlc_val, epidsc_val, sp_sc_err): 
 ...
 for j in range(length_x - 1):
  if pt_bin == len(pt_bins):
   y_bin += 1
   pt_bin = 1
   print(str("\hline"))# some stuff for latex table
  ...
  print("(" + str(data[j + 1, 3]) +"," + str(data[j + 1, 4]) + ") & "+ srtformat.format(r) + " & "+ srtformat.format(s1) + " & " + srtformat.format(s2) + " & " + srtformat.format(s3) + " & " + srtformat.format(summ)  +"\\" + "\\") #fill latex table
...
if __name__ == "__main__" :
 ...
 h_canvas_1 = ROOT.TCanvas("h1", "Bin_values", 800, 600)
 ...
 h_canvas_2 = ROOT.TCanvas("h2", "Bin_values", 800, 600)
 h_canvas_2.cd()
